import { Wind, Settings2, UtensilsCrossed, Home, Wrench } from 'lucide-react';
                  ...
                  <ul className="flex flex-col gap-2.5 flex-1">
                    {plan.features.map((feat, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-[0.875rem] text-muted">
                        <span className="w-4 h-4 rounded-full border-2 border-primary/40 bg-primary-dim flex-shrink-0 mt-0.5" />
                        <span>{feat}</span>
                      </li>
                    ))}
                  </ul>
